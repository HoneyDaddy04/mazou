...
export function Topbar() {
  ...
  return (
    <>
      <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-bg shrink-0">
        <div className="flex items-center gap-3.5">
          ...
          <div ref={dateRef} className="relative">
            ...
            {dateOpen && (
              <div className="absolute top-full left-0 mt-1 bg-surface border border-border rounded-lg shadow-xl z-30 py-1 min-w-[180px]">
                {dateRanges.map((r) => (
                  <button
                    key={r.label}
                    onClick={() => { setDateRange(r); setDateOpen(false); toast(`Date range: ${r.label}`); }}
                    className={`w-full text-left px-3 py-2 text-xs hover:bg-surface-2 transition-colors cursor-pointer ${
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="bg-accent border-none text-black px-3 py-[5px] rounded-md text-xs font-semibold cursor-pointer font-sans hover:brightness-110 transition-all"
          >
            + Add API Key
          </button>
        </div>
      ...
      <Modal open={showNewKey} onClose={handleCloseKeyModal} title="Create API Key" icon="🔑">
        {!createdKey ? (
