...
import { isDemoActive, MOCK_DASHBOARD_STATS, MOCK_USAGE_TIMESERIES, MOCK_RECOMMENDATIONS, MOCK_AGENTS, MOCK_WALLET, MOCK_WALLET_TRANSACTIONS, MOCK_INVOICES, MOCK_KEYS, MOCK_MODELS, MOCK_ROUTING_RULES, MOCK_USER } from "./demo-auth";
...
export function useDashboardStats(days = 30) {
  return useQuery({
    ...
    queryFn: () => isDemoActive() ? MOCK_DASHBOARD_STATS : HAS_BACKEND ? fetcher(dashboardApi.stats(days)) : sq.fetchDashboardStats(days),
  });
}
...
export function useDashboardUsage(days = 30, groupBy = "day") {
  return useQuery({
    ...
    queryFn: () => isDemoActive() ? MOCK_USAGE_TIMESERIES : HAS_BACKEND ? fetcher(dashboardApi.usage(days, groupBy)) : sq.fetchUsageTimeseries(days, groupBy),
  });
}
...
export function useRecommendations() {
  return useQuery({
    ...
    queryFn: () => isDemoActive() ? MOCK_RECOMMENDATIONS : HAS_BACKEND ? fetcher(dashboardApi.recommendations) : sq.fetchRecommendations(),
  });
}
...
export function useAgents() {
  return useQuery({
    queryKey: ["agents"],
    queryFn: () => isDemoActive() ? MOCK_AGENTS : HAS_BACKEND ? fetcher(dashboardApi.agents) : sq.fetchAgents(),
  });
}
...
export function useWallet() {
  return useQuery({
    queryKey: ["wallet"],
    queryFn: () => isDemoActive() ? MOCK_WALLET : HAS_BACKEND ? fetcher(dashboardApi.wallet) : sq.fetchWallet(),
  });
}
...
export function useWalletTransactions(limit = 20, offset = 0) {
  return useQuery({
    ...
    queryFn: () => isDemoActive() ? MOCK_WALLET_TRANSACTIONS : HAS_BACKEND ? fetcher(dashboardApi.walletTransactions(limit, offset)) : sq.fetchWalletTransactions(limit, offset),
  });
}
...
export function useInvoices() {
  return useQuery({
    queryKey: ["invoices"],
    queryFn: () => isDemoActive() ? MOCK_INVOICES : HAS_BACKEND ? fetcher(dashboardApi.invoices) : sq.fetchInvoices(),
  });
}
...
export function useDepartments(days = 30) {
  return useQuery({
    ...
    queryFn: () => isDemoActive() ? MOCK_DASHBOARD_STATS.features : HAS_BACKEND ? fetcher(dashboardApi.departments(days)) : sq.fetchDashboardStats(days).then(s => s.features),
  });
}
...
export function useApiKeys() {
  return useQuery({
    queryKey: ["keys"],
    queryFn: () => isDemoActive() ? MOCK_KEYS : HAS_BACKEND ? fetcher(keysApi.list) : sq.fetchApiKeys(),
  });
}
...
export function useModels(params?: { is_african?: boolean; category?: string }) {
  return useQuery({
    ...
    queryFn: () => isDemoActive() ? MOCK_MODELS.filter(m => params?.is_african !== undefined ? m.is_african === params.is_african : true) : HAS_BACKEND ? fetcher(modelsApi.list(params)) : Promise.resolve(sq.fetchModels(params)),
  });
}
...
export function useRoutingRules() {
  return useQuery({
    ...
    queryFn: () => isDemoActive() ? MOCK_ROUTING_RULES : HAS_BACKEND ? fetcher(routingApi.rules) : sq.fetchRoutingRules(),
  });
}
...
export function useCurrentUser() {
  return useQuery({
    queryKey: ["auth", "me"],
    queryFn: () => isDemoActive() ? MOCK_USER : HAS_BACKEND ? fetcher("/api/auth/me") : sq.fetchCurrentUser(),
    retry: false,
  });
}
